...
removed_items = sum_init_matrix - sum_sum_matrix
all_removed = removed_items

while removed_items >= 1:
    prev_sum_sum_matrix = sum_sum_matrix
    prev_sum_matrix = sum_matrix.copy()
    sum_matrix = convolve2d(prev_sum_matrix, kernel, mode='same', boundary='fill', fillvalue=0)
    sum_matrix = [[0 if prev_sum_matrix[i][j] == 0 or sum_matrix[i][j] < 4 else 1 
               for j in range(len(sum_matrix[0]))] for i in range(len(sum_matrix))]
    sum_sum_matrix = sum([sum(x) for x in sum_matrix])
    removed_items = prev_sum_sum_matrix - sum_sum_matrix
    all_removed += removed_items
